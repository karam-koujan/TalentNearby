...
const Profile = ({
  ...
  const [enableElementModification, setEnableElementModification] =
    React.useState({
      phoneNumber,
      bio,
    });
  ...
  const infoMutation = useMutation((newData) =>
    setUpdate("http://localhost:8080/api/profile/modifyInfo", newData)
  );
  const profileImgMutation = useMutation((newData) =>
    setUpdate("http://localhost:8080/api/profile/updateProfileImg", newData)
  );
      ...
      <Wrapper>
        ...
        >
          {profileImg || uploadedImg ? (
            <img
              loading="lazy"
              src={uploadedImg ? uploadedImg : profileImg}
              alt={`${userName} image`}
            />
          ) : null}
          {uploadedImg ? (
            <div className={Styles.uploadImgIconsWrapper}>
              ...
              <UpdateProfileImgBtn onClick={handleUpdateProfileImg}>
                <ChangeProfileImgIcon className="fa fa-check"></ChangeProfileImgIcon>
              </UpdateProfileImgBtn>
            </div>
          ) : null}
          <UploadImgElementsWrapper>
        ...
        <DescriptionList>
          {status === "talent" ? (
            <>
              <DescriptionTitle>job</DescriptionTitle>
              <Discription>{job}</Discription>
            </>
          ) : null}
          <DescriptionTitle>Phone</DescriptionTitle>
            ...
            <div className={Styles.inputWrapper}>
              ...
              {phoneNumber ? (
                <Close
                  ...
                  onClick={() =>
                    setEnableElementModification((prev) => ({
                      ...prev,
                      phoneNumber: true,
                    }))
                  }
          ...
          <DescriptionTitle>bio</DescriptionTitle>
          {enableElementModification.bio ? (
            <div style={{ display: "flex" }}>
              <Discription>{bio}</Discription>
              <Pencil
                className="fa fa-pencil"
                onClick={() =>
                  setEnableElementModification({
