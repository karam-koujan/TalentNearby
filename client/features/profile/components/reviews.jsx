...
const Reviews = ({ profileId }) => {
  const { data, status, fetchMore, isFetchingNextPage, hasNextPage } =
    useFetchInfiniteQuery(
      ["reviews", profileId],
      `http://localhost:8080/api/profile/reviews/${profileId}`,
      2
    );
        ...
        {status === "loading"
          ...
          : data.pages.map((page, currentPageIdx) => (
              <React.Fragment key={currentPageIdx}>
                {!data.pages.every((page) => page.reviews.length === 0) ? (
                  page.reviews.map(
                    (
                      { text, rating, reviewer: { userName, profileImg } },
                      currentReview
                    ) => (
                      <React.Fragment key={currentReview}>
                        <ReviewWrapper>
                          ...
                          <ReviewerText>{text}</ReviewerText>
                        </ReviewWrapper>
                        {currentReview === page.reviews.length - 2 &&
                        hasNextPage ? (
