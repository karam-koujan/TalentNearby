...
exports.isVerificationCodeExist = (email) => {
  return new Promise((resolve, reject) => {
    VerificationCode.findOne({ email }, (err, result) => {
      ...
      if (result)
        return reject(
          new AuthorizationError("the code already sent check your inbox")
        );
      return resolve();
    });
  });
...
exports.isEmailVerified = (email, Model) => {
  return new Promise((resolve, reject) => {
    Model.findOne({ email }, (err, user) => {
      if (err) return reject();
      if (!user)
        return reject(new AuthorizationError("this email doesn't exist"));
...
exports.codeVerificationProvider = (body) => {
  ...
  return new Promise((resolve, reject) => {
    VerificationCode.findOne({ email, code }, (err, result) => {
      ...
      if (!result)
        return reject(
          new AuthorizationError("The code is uncorrect or it expires")
        );
      return resolve();
    });
  });
